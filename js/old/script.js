...
function dump(obj, level) {
	...
	if (app.verbosity < level) return;
	console.log(obj)
}
/*!
Thx to Dmitry Artamoshkin - hamatata.com
*/
var app = {
...
function process_ya_reply(data, text) {
	...
	if (data.length > 0) {
		res += '<a class="btn btn-small btn-success" id="yandex_btn" onclick="hideUrban();">Yandex</a>';
		// res += '<a class="btn btn-small btn-success" id="urban_btn" onclick="hideYandex();">Urban</a>';
		res += '<a href="#urban-data" class="btn btn-small btn-success" id="urban_btn" onclick="hideYandex();">Urban</a>';
...
function baloon_no_extension_notice() {
	...
	if (app.is_mobile) res += "<h5>Расширение недоступно для мобильных устройств и планшетов</h5>";
	else if (app.not_chrome) res += "<p>Расширение доступно для браузеров <strong>Firefox</strong>, <strong>Chrome</strong> и других аналогов на основе <strong>Chromium</strong> (Opera, Яндекс.Браузер и т.д.)</p><br/>" + '<a class="btn btn-success" href="page/help#browsers">Выбрать расширение</a>';
	else res += '<a class="btn btn-success ext-install">Добавить расширение</a>';
...
// function check_extension() {
	// if (app.has_extension) return true;
...
// function check_extension_loop() {
	// if (!check_extension()) {
		// var tries = 50;
		// var timer = setInterval(function() {
			// if (check_extension() || tries-- <= 0) clearInterval(timer)
		// }, 200)
	// }
// }
...
function check_video_type() {
	if ($("#video_type").val().match(/^(mp4|webm|ogg|flv)$/)) return true;
	...
	if ($("input[name=video_src]").val() === "") {
		...
		var src = $("input[name=local_video_mobile]").val();
	}
	if (src == "") {
		return false
	}
...
function generate_player_code(video_src, video_type, subs_url) {
	if (video_type === "flv") video_type = "flash";
...
function update_video_attrs() {
	...
	video_attrs.subs_url = $("#subs_url").val();
...
function unknown_words_declension(n) {
	if (n > 10 && n < 15) return "незнакомых слов";
	switch (n % 10) {
		case 1:
...
function copyAll() {
	var val = $('#words_list').html();
	window.location='tg://msg_url?url='+encodeURIComponent(val);
	prompt('Скопируйте слова ниже или Ctrl+C', val);
}
/* video_attrs http://localhost:8080/1.srt 
let = (msg) => {
}
*/
/* Some Fix + Mobile - Konovalov Kirill*/
function checkIfLandscape() {
	if (document.documentElement.clientWidth > document.documentElement.clientHeight) {
		return true
	} else {
		alert('Для большего комфорта поменяйте режим экрана на ландшафтный')
	}
}
/* Для Android*/
function ifMobile() {
...
/* change get_translation on 2999 3007 3028*/